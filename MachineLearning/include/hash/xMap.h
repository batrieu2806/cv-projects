...
V xMap<K,V>::put(K key, V value){
    int index = this->hashCode(key, capacity); // Compute the index
    V retValue = value;

    // Retrieve the linked list from the table at the computed index
...
DLinkedList<int> xMap<K,V>::clashes(){
    ...
    for (int i = 0; i < capacity; i++) {
        DLinkedList<Entry*>& list = this->table[i];
        int collisionCount = (list.size() > 1) ? list.size() : 0;
