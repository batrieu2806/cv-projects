...
void XArrayList<T>::clear()
{
    // TODO
    if (this->deleteUserData) {
        this->deleteUserData(this);  // Call the custom delete function if provided
    }

    this->count = 0;  // Set count to 0, effectively "clearing" the list
}
...
string XArrayList<T>::toString(string (*item2str)(T &))
{
    ...
    result += "]";  // End with the closing bracket
...
void XArrayList<T>::checkIndex(int index)
{
    ...
    // TODO
    if (index < 0) {
        throw std::out_of_range("Index is out of range: cannot be negative.");
    }
    if (index > this->count){
           throw std::out_of_range("Index is out of range: exceeds the number of elements.");
    }
}
template <class T>
