...
double_tensor MLPClassifier::predict(
    ...
    for(auto batch: *pLoader){
        ...
        for (auto pLayer : m_layers) {
            X = pLayer->forward(X); // Forward pass for batch
        }
        ...
        if (first_batch) {
            ...
            results = xt::concatenate(xt::xtuple(results, X), 0);
        }
...
double_tensor MLPClassifier::forward(double_tensor X){
   ...
   size_t i = 0;
    for (auto it = m_layers.begin(); it != m_layers.end(); ++it) {
        ...
        // Print input shape
        //std::cout << "Forward Pass - Layer " << i + 1 << ": Input shape = [";
       // for (size_t dim : X.shape()) {
           // std::cout << dim << " ";
        //}
        //std::cout << "]" << std::endl;
        ...
        // Print output shape
        //std::cout << "Forward Pass - Layer " << i + 1 << ": Output shape = [";
        //for (size_t dim : X.shape()) {
         //   std::cout << dim << " ";
       // }
        //std::cout << "]" << std::endl;
